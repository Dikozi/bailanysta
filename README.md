...
Социальная сеть: лента постов, профили, друзья, личные сообщения, лайки,
комментарии, поиск
...
постами, друзьями, перепиской и уведомлениями.
...
3. **Расширенная соцсеть**: создание, редактирование и удаление постов;
   заявки в друзья с подтверждением; личные сообщения между друзьями;
   статус в профиле; уведомления о лайках, комментариях и заявках
...
Seed создаёт 10 пользователей, 46 постов, лайки, комментарии, дружеские
связи, переписки и уведомления. Все демо-аккаунты имеют пароль `demo12345`.
...
| `GET` | `/api/v1/posts` | Лента. Параметры: `cursor`, `limit`, `feed=global\|friends`, `authorId`, `tag`, `q` |
...
| `POST` `DELETE` | `/api/v1/users/:username/friend-request` | Отправить заявку · отменить заявку или удалить из друзей |
| `POST` | `/api/v1/users/:username/friend-request/accept` · `/decline` | Принять или отклонить входящую заявку |
| `GET` | `/api/v1/users/:username/friends` | Список друзей |
| `GET` | `/api/v1/messages/conversations` · `/unread` | Список бесед и счётчик непрочитанных |
| `GET` `POST` | `/api/v1/messages/:username` | Переписка: читать и отправлять |
| `POST` | `/api/v1/messages/:username/read` | Отметить беседу прочитанной |
...
| **Instagram** | Ряд авторов с кольцами наверху, «взрыв» сердца при лайке, профиль плиткой. | Ряд даёт ленте единственное цветное пятно и превращает список людей из абстракции в лица. Плитка отличает профиль от ленты. |
...
### Дружба хранится одной строкой, а не двумя

Изначально в проекте была односторонняя подписка (как в Twitter). Её заменила
взаимная дружба с подтверждением (как в Facebook) — и вместе с ней исчезла
вторая параллельная система связей, которая только путала бы.

Ключевое решение: `FriendRequest` — **одна направленная строка** «кто кому
написал первым» со статусом `PENDING → ACCEPTED`. Отдельной симметричной
таблицы `Friendship` нет: принятие заявки — это `UPDATE` одной строки,
а не удаление записи из одной таблицы и вставка в другую. «Кто с кем дружит» —
это `OR` по обеим сторонам строки со статусом `ACCEPTED`.

Отдельно обработан случай встречных заявок: если A позвал B, а B, не увидев
этого, зовёт A — вторая заявка не создаётся, а **сразу принимается первая**.
Это и есть согласие обеих сторон; создавать два конфликтующих ряда было бы
и неверно, и потребовало бы разбираться, какой из них главный.

### Мессенджер без таблицы «беседа»

`Message` хранит только `senderId`, `receiverId` и текст. Сущности
`Conversation` нет — беседа выводится из пары собеседников. Отдельная таблица
оправдана в мессенджере с групповыми чатами; здесь только личные 1:1, и
заводить ради этого лишнюю сущность значило бы усложнить схему без выгоды.

Список бесед — единственное место в проекте с сырым SQL: «последнее сообщение
в каждой группе» обычным запросом Prisma не выразить, нужен `DISTINCT ON`.
Пара канонизируется через `LEAST/GREATEST`, чтобы (A,B) и (B,A) считались одной
беседой независимо от того, кто написал последним.

Право на переписку проверяется **на сервере при каждом запросе**, а не только
скрытием кнопки: попытка написать не-другу возвращает `403` даже из curl.
...
в ленте друзей, в профиле автора, на своей странице. Правится он **везде**:
