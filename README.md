...
2. **AI-помощник не проверен с реальным ключом.** Код написан по актуальной
   документации Anthropic SDK и полностью проверен на пути «ключа нет →
   честный `503` → понятное сообщение в интерфейсе». Сам вызов модели
   не выполнялся ни разу: ключа на момент разработки не было. Если при
   первом запуске с ключом API вернёт ошибку по параметрам запроса,
   смотреть нужно в `src/server/ai/anthropic.ts` — в частности, на
   `betas` и `fallbacks` (страховка от отказа модели). Их можно убрать
   без последствий для остальной логики.
